The App Store privacy policy URL is `https://streamtrack.gocloudlaunch.com/privacy-policy`.
...
* **SETUP**:
    ...
    * For CLOUD EAS Expo Builds (`npx eas build -p ios --profile production`):
        ...
        * Add variable `GOOGLE_SERVICES_PLIST` with the **file** upload button as Sensitive in both dev and production
...
In `StreamTrack/APP/.env.local`, change `DATA_API_URL` from localhost to the computer's IP address for LAN testing, or to `https://streamtrack.gocloudlaunch.com/` for production.
...
Choose `Sensitive` because Secret will hide it forever

For file secrets, like `GOOGLE_SERVICES_PLIST`:
* Go to the [Expo EAS website](https://expo.dev)
* Go to your project
* Go to environmental variables under project settings
